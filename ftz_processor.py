...
# alongside it, still runs.
try:
    from _version import OCEAN_VERSION as __version__
except Exception:
    __version__ = "0.0.0"
...
def main() -> int:
    ...
    print(f"Unimex FTZ Processor  v{__version__}")
    ...
    if getattr(sys, "frozen", False) and problems > 0:
        try:
            ...
            updated = updater.check_and_update("ocean", "UnimexFTZ.exe", __version__)
        except Exception as e:
